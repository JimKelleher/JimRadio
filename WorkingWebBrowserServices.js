...
tab.prototype.show_tab_set = function (selected_tab) {
    ...
    switch (selected_tab.id) {
        ...
        case this.tab_name + "Tab1":
            this.tab_content.innerHTML = document.getElementById(
                this.tab_name + "Tab1Content").innerHTML;
            break;
        case this.tab_name + "Tab2":
            this.tab_content.innerHTML = document.getElementById(
                this.tab_name + "Tab2Content").innerHTML;
            break;
        case this.tab_name + "Tab3":
            this.tab_content.innerHTML = document.getElementById(
                this.tab_name + "Tab3Content").innerHTML;
            break;
        default:
            this.tab_content.innerHTML = document.getElementById(
                this.tab_name + "Tab1Content").innerHTML;
...
function get_array_pages(total_results, results_per_page) {
    ...
    if (total_pages_remainder > 0) { total_pages++; }
...
function get_xml_http_request_via_proxy_server(uri) {
    ...
    var PROXY_SERVER_PREFIX = "http://www.workingweb.info/Utility/WorkingWebWebServices/api/GetWebRequestResponse/?uriStringEncoded=";
...
function remove_response_packaging(response) {
    ...
    if (response_edited.indexOf("<string xmlns=") > -1) {
        ...
        response_edited = excise_bounds_inclusive(response_edited, "<string xmlns=", '/">')
  ...
  //var FIREFOX_ERROR            = "JSON.parse: bad control character in string literal";
    ...
    try {
        ...
        if (error_message.indexOf("at line 1 column") == -1) {
            error_message = "JSON.parse error.  Use FireFox to find the details.";
        } else {
            ...
            var JSON_string_focus_on_error =
                JSON_string.substring(
                    error_column - CHARACTER_BUFFER_LENGTH,
                    error_column + CHARACTER_BUFFER_LENGTH
                );
        ...
        // NOTE: We use alert because the process will hang somewhere down the line and we will never
...
function get_next_year_index(look_in, start_at) {
    ...
    if (candidate_year_index > -1) {
        ...
        candidate_year = look_in.substr(candidate_year_index, 4);
...
function get_seconds_from_ISO_duration(duration_string) {
    ...
    if (duration_string_edited.indexOf("H") == -1) { duration_string_edited = "0H" + duration_string_edited; }  // pre-pend
    if (duration_string_edited.indexOf("S") == -1) { duration_string_edited += "0S"; }                          // append
    if (duration_string_edited.indexOf("M") == -1) { duration_string_edited += "0M"; }                          // append to wrong position
...
function remove_HTML_comments(look_in, debug) {
    ...
    if (look_in_edited.indexOf(HTML_COMMENT_START_DUMMY) > -1) {
        ...
        look_in_edited = excise_bounds_inclusive(

            look_in_edited,
            HTML_COMMENT_START_DUMMY,   // though these say "dummy", they are used for production
...
function excise_bounds_inclusive(look_in, start_token, end_token, debug) {
    ...
    // Built-in debugging:
    if (debug == true) { print_string("DEBUG EXCISION BEFORE: " + look_in_edited); }
    ...
    do {
        ...
        if (cut_start_index > -1) {
            ...
            if (cut_end_index > -1) {
                ...
                // Built-in debugging:
                if (debug == true) { print_string("DEBUG EXCISION BOUNDED STRING: " + represent_bounded_string_succinctly(look_in_edited, cut_start_index, cut_end_index)); }
    ...
    // Built-in debugging:
    if (debug == true) { print_string("DEBUG EXCISION AFTER: " + look_in_edited); }
...
function open_popup_window(window_uri, modal_dialog,
    resizable, scrollbars, width_in_pixels, height_in_pixels) {

    //----------------------------------------------------------------------
    // NOTE: This is "feature sensing" in lieu of browser detection.
    // If modal dialogs are supported by the browser:
    var use_modal_dialog = false;

    if (window.showModalDialog) {

        // If the calling module wants a modal dialog: 
        if (modal_dialog == true) {

            // NOTE: True handles all other browsers:
    ...
    if (use_modal_dialog == true) {

        // Use the colon for equality and the semicolon as a separator:
        ...
        window_desc += "dialogWidth:" + width_in_pixels + "px;";
        window_desc += "dialogHeight:" + height_in_pixels + "px;";

        window_desc += "dialogLeft:" + x.toString() + ";";
        window_desc += "dialogTop:" + y.toString();
        ...
        window_desc += "width=" + width_in_pixels + "px,";
        window_desc += "height=" + height_in_pixels + "px,";
    ...
    // Browser              Window Type     Favicon Shows   Comments
    // -------              -----------     -------------   --------

    // Brave                regular         yes
    // Chrome               regular         yes
    // Vivaldi              regular         yes             opens in its own tab,
    //                                                      not a popup
    // Edge                 regular         no

    // Firefox              modal           no
    // Internet Explorer    modal           no
    // Opera                modal           no
    // Safari               modal           no
    ...
    if (use_modal_dialog == true) {
        ...
        window_object = window.showModalDialog(window_uri, "_blank", window_desc);
    }
    else {

        // Open a regular window:
        window_object = window.open(window_uri, "_blank", window_desc);
    }
...
function set_window_title(window_to_set, title) {
    ...
    if (window_to_set.document) {
        ...
        // NOTE: Recursive call:
        setTimeout(set_window_title(window_to_set, title), 10); // milliseconds
    }
